...
/**
 * `querypad explain [folder]`: read the inferred relationships from `.querypad/` and
...
export async function runExplain(folder: string, options: RunExplainOptions = {}): Promise<number> {
  ...
  const { relationships, profiles } = await readArtifacts(folder);
  if (!relationships) {
    log(
      `No .querypad/relationships.json found in ${folder}. ` +
